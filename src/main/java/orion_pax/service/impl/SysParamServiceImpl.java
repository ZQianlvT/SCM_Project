		...
		for (SysParam sysParam : sysParamList) {
			...
			if ("1".equals(sysParam.getSysParamType())) {
				...
				for (SysParam sysParam2 : selectOtherTableList) {
					fieldMap.put(sysParam2.getSysParamValue(),
							sysParam2.getSysParamText());
				}
				// 把字段的map存入到系统参数的map
				...
				if (sysParamMap.get(sysParam.getSysParamField()) == null) {
					fieldMap = new HashMap<String, Object>();
					// 先存放该字段的值到字段的map
					fieldMap.put(sysParam.getSysParamValue(),
							sysParam.getSysParamText());
					// 再把字段的map存放到系统的参数map
					sysParamMap.put(sysParam.getSysParamField(), fieldMap);
				} else {
					fieldMap = (Map<String, Object>) sysParamMap.get(sysParam
							.getSysParamField());
					fieldMap.put(sysParam.getSysParamValue(),
							sysParam.getSysParamText());
				}
